...
define [], () ->
  ->
    ...
    @onEnterSearch = (event, from, to, orderForm, rules, usePostalCodeSearch) ->
      @attr.data.active = true
      console.log "Enter search"
      @select('addressSearchSelector').trigger('enable.vtex', [orderForm, rules, if usePostalCodeSearch? then usePostalCodeSearch or true]])
      @select('shippingOptionsSelector').trigger('disable.vtex')
      @select('goToPaymentButtonWrapperSelector').hide()
